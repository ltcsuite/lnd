...
func (n *NetworkHarness) SetUp(t *testing.T,
	...
	for _, client := range clients {
		for i := 0; i < 10; i++ {
			...
			addr, err := ltcutil.DecodeAddress(resp.Address, n.netParams)
			if err != nil {
				return err
			}
	...
	expectedBalance := int64(ltcutil.SatoshiPerBitcoin * 10)
...
func (n *NetworkHarness) OpenPendingChannel(srcNode, destNode *HarnessNode,
	amt ltcutil.Amount,
	pushAmt ltcutil.Amount) (*lnrpc.PendingUpdate, error) {
...
func (n *NetworkHarness) SendCoins(t *testing.T, amt ltcutil.Amount,
	target *HarnessNode) {
...
func (n *NetworkHarness) SendCoinsUnconfirmed(t *testing.T, amt ltcutil.Amount,
	target *HarnessNode) {
...
func (n *NetworkHarness) SendCoinsNP2WKH(t *testing.T, amt ltcutil.Amount,
	target *HarnessNode) {
...
func (n *NetworkHarness) sendCoins(amt ltcutil.Amount, target *HarnessNode,
	...
	// and exit.
	if !confirmed {
		expectedBalance := ltcutil.Amount(initialBalance.UnconfirmedBalance) + amt
	...
	expectedBalance := ltcutil.Amount(fullInitialBalance) + amt
