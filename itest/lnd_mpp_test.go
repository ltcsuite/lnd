...
func newMppTestScenario(ht *lntest.HarnessTest) *mppTestScenario {
	...
	for i := 0; i < 2; i++ {
		...
		ht.FundCoinsUnconfirmed(ltcutil.SatoshiPerBitcoin, eve)
		ht.MineBlocks(1)
	}
...
type mppOpenChannelRequest struct {
	// Channel Alice=>Carol.
	amtAliceCarol ltcutil.Amount

	// Channel Alice=>Dave.
	amtAliceDave ltcutil.Amount

	// Channel Carol=>Bob.
	amtCarolBob ltcutil.Amount

	// Channel Carol=>Eve.
	amtCarolEve ltcutil.Amount

	// Channel Dave=>Bob.
	amtDaveBob ltcutil.Amount

	// Channel Eve=>Bob.
	amtEveBob ltcutil.Amount
}
...
func (m *mppTestScenario) buildRoute(amt ltcutil.Amount,
