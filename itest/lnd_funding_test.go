...
func testBasicChannelFunding(ht *lntest.HarnessTest) {
	...
	testFunding := func(ht *lntest.HarnessTest, carolCommitType,
		...
		// fund the channel.
		ht.FundCoins(ltcutil.SatoshiPerBitcoin, carol)
		...
		if carolCommitType == lnrpc.CommitmentType_SIMPLE_TAPROOT &&
			...
			expectedErr := fmt.Errorf("requested channel type " +
				"not supported")
			amt := funding.MaxLtcFundingAmount
...
func basicChannelFundingTest(ht *lntest.HarnessTest,
	...
	*lnrpc.Channel, func()) {
	...
	chanAmt := funding.MaxLtcFundingAmount
	pushAmt := ltcutil.Amount(100000)
	satPerVbyte := ltcutil.Amount(1)
...
func testUnconfirmedChannelFunding(ht *lntest.HarnessTest) {
	...
	// response to a channel balance RPC.
	checkChannelBalance := func(node *node.HarnessNode,
		local, remote, pendingLocal, pendingRemote ltcutil.Amount) {
...
func runChannelFundingInputTypes(ht *lntest.HarnessTest, alice,
	...
	const (
		chanAmt  = funding.MaxLtcFundingAmount
		burnAddr = "rltc1qxsnqpdc842lu8c0xlllgvejt6rhy49u6hkmpnu"
	)

	fundMixed := func(amt ltcutil.Amount, target *node.HarnessNode) {
	...
	fundMultipleP2TR := func(amt ltcutil.Amount, target *node.HarnessNode) {
		ht.FundCoinsP2TR(amt/4, target)
		ht.FundCoinsP2TR(amt/4, target)
		ht.FundCoinsP2TR(amt/4, target)
		ht.FundCoinsP2TR(amt/4, target)
	}
	fundWithTypes := []func(amt ltcutil.Amount, target *node.HarnessNode){
	...
	checkChannelBalance := func(node *node.HarnessNode, local,
		remote, pendingLocal, pendingRemote ltcutil.Amount) {
...
func runExternalFundingScriptEnforced(ht *lntest.HarnessTest) {
	...
	ht.FundCoins(ltcutil.SatoshiPerBitcoin*20, carol)
	...
	const chanSize = funding.MaxLtcFundingAmount
...
func testChannelFundingPersistence(ht *lntest.HarnessTest) {
	...
	const numConfs = 5
	carolArgs := []string{
		fmt.Sprintf("--litecoin.defaultchanconfs=%v", numConfs),
	}
	carol := ht.NewNode("Carol", carolArgs)
...
func testBatchChanFunding(ht *lntest.HarnessTest) {
	...
	ensurePolicy(
		...
		chainreg.DefaultLitecoinFeeRate,
	)
	ensurePolicy(
		ht, alice, carol, chanPoint2,
		chainreg.DefaultLitecoinBaseFeeMSat,
...
func deriveFundingShim(ht *lntest.HarnessTest, carol, dave *node.HarnessNode,
	chanSize ltcutil.Amount, thawHeight uint32, publish bool,
