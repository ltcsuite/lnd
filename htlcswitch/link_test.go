...
import (
	...
	invpkg "github.com/ltcsuite/lnd/invoices"
...
func TestChannelLinkRevThenSig(t *testing.T) {
	...
	const chanAmt = ltcutil.SatoshiPerBitcoin * 5
	const chanReserve = ltcutil.SatoshiPerBitcoin * 1
...
func TestChannelLinkSingleHopPayment(t *testing.T) {
	...
	alice, bob, err := createTwoClusterChannels(
		t, ltcutil.SatoshiPerBitcoin*3, ltcutil.SatoshiPerBitcoin*5,
	)
	...
	amount := lnwire.NewMSatFromSatoshis(ltcutil.SatoshiPerBitcoin)
...
func TestExitNodeHTLCTimelockExceedsPayload(t *testing.T) {
	...
	channels, _, err := createClusterChannels(
		t, ltcutil.SatoshiPerBitcoin*5, ltcutil.SatoshiPerBitcoin*5,
	)
	require.NoError(t, err, "unable to create channel")
	...
	const amount = ltcutil.SatoshiPerBitcoin
...
func TestExitNodeHTLCUnderpaysPayloadAmount(t *testing.T) {
	...
	channels, _, err := createClusterChannels(
		t, ltcutil.SatoshiPerBitcoin*5,
		ltcutil.SatoshiPerBitcoin*5,
	)
	require.NoError(t, err, "unable to create channel")
	...
	f := func(underpaymentRand uint64) bool {
		const amount = ltcutil.SatoshiPerBitcoin / 100
...
func TestLinkForwardFeePolicyMismatch(t *testing.T) {
	...
	amountNoFee := lnwire.NewMSatFromSatoshis(ltcutil.SatoshiPerBitcoin)
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	// rejected.
	amount := lnwire.NewMSatFromSatoshis(4 * ltcutil.SatoshiPerBitcoin)
	htlcAmt, totalTimelock, hops := generateHops(amount, testStartingHeight,
		n.firstBobChannelLink, n.carolChannelLink)
...
func newSingleLinkTestHarness(t *testing.T, chanAmt, chanReserve ltcutil.Amount) (
...
func TestChannelLinkTrimCircuitsPending(t *testing.T) {
	...
	const (
		chanAmt   = ltcutil.SatoshiPerBitcoin * 5
		numHtlcs  = 4
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	// a switch initiated payment.  The resulting bandwidth should
	// now be decremented to reflect the new HTLC.
	htlcAmt := lnwire.NewMSatFromSatoshis(3 * ltcutil.SatoshiPerBitcoin)
	...
	const bobChanAmt = ltcutil.SatoshiPerBitcoin * 1
	const bobChanReserve = ltcutil.SatoshiPerBitcoin * 1.5
...
func TestChannelLinkUpdateCommitFee(t *testing.T) {
	...
	// for this test.
	const aliceInitialBalance = ltcutil.SatoshiPerBitcoin * 3
	channels, _, err := createClusterChannels(
		t, aliceInitialBalance, ltcutil.SatoshiPerBitcoin*5,
	)
	require.NoError(t, err, "unable to create channel")
