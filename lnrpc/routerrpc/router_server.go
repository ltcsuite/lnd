...
func (s *Server) EstimateRouteFee(ctx context.Context,
	...
	// native unit of LN.
	amtMsat := lnwire.NewMSatFromSatoshis(ltcutil.Amount(req.AmtSat))

	// Pick a fee limit
	//
	// TODO: Change this into behaviour that makes more sense.
	feeLimit := lnwire.NewMSatFromSatoshis(ltcutil.SatoshiPerBitcoin)
...
func toPairSnapshot(pairResult *PairHistory) (*routing.MissionControlPairSnapshot,
	...
	failAmt, failTime, err := getPair(
		...
		ltcutil.Amount(pairResult.History.FailAmtSat),
	...
	successAmt, successTime, err := getPair(
		...
		ltcutil.Amount(pairResult.History.SuccessAmtSat),
...
func getPair(amtMsat lnwire.MilliSatoshi, amtSat ltcutil.Amount,
...
func getMsatPairValue(msatValue lnwire.MilliSatoshi,
	satValue ltcutil.Amount) (lnwire.MilliSatoshi, error) {
