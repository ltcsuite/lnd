...
type chanFundMaxTestCase struct {
	...
	initialWalletBalance ltcutil.Amount
	...
	expectedBalanceAlice ltcutil.Amount
	...
	var testCases = []*chanFundMaxTestCase{
		{
			...
			initialWalletBalance: 3_000,
			chanOpenShouldFail:   true,
			feeRate:              20,
			expectedErrStr: "output amount(0.00000565 LTC) " +
				"after subtracting fees(0.00002435 LTC) " +
				"below dust limit(0.00003300 LTC)",
		},
		{
		...
		},
		{
			...
			expectedBalanceAlice: ltcutil.Amount(37_000) -
				fundingFee(1, false),
		},
		{
		...
		// balance.
		{
			...
			expectedErrStr: "funder balance too small (-8050000) " +
				"with fee=9050 sat, minimum=10920 sat required",
		},
		{
			name: "wallet amount > max chan size, " +
...

		{
			...
			expectedBalanceAlice: ltcutil.Amount(100_000) -
...
func runFundMaxTestCase(ht *lntest.HarnessTest, alice, bob *node.HarnessNode,
	testCase *chanFundMaxTestCase, reserveAmount ltcutil.Amount) {

	ht.FundCoins(testCase.initialWalletBalance, alice)

	defer func() {
		if testCase.initialWalletBalance <= 3_000 {
...
// wallet.
func fundingFee(numInput int, change bool) ltcutil.Amount {
