...
OPTIONS_NO_LND_DEFAULT_VALUE_CHECK="adminmacaroonpath readonlymacaroonpath \
    ...
    litecoin.chaindir litecoin.defaultchanconfs litecoin.defaultremotedelay \
    litecoin.dnsseed litecoin.chaindir litecoin.dnsseed \
...
for OPTION in $LND_OPTIONS; do
    ...
    if [[ "$OPTION" == *"="* ]]; then
        ...
        DEFAULT_VALUE_LND="$(echo $LND_HELP | \
            ...
            -e "s#m0s#m#g" \
            -e "s#h0m#h#g" \
            -e "s#$HOME/Library/Application Support/Lndltc#~/.lnd#g" \
