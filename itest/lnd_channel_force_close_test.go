...
func testChannelForceClosure(ht *lntest.HarnessTest) {
	...
	for _, channelType := range commitTypes {
		...
		success := ht.Run(testName, func(t *testing.T) {
			...
			st.FundCoins(ltcutil.SatoshiPerBitcoin, alice)

			// Also give Carol some coins to allow her to sweep her
			// anchor.
			st.FundCoins(ltcutil.SatoshiPerBitcoin, carol)
...
func channelForceClosureTest(ht *lntest.HarnessTest,
	...
	defaultCLTV := uint32(chainreg.DefaultLitecoinTimeLockDelta)
	...
	for i := 0; i < numInvoices; i++ {
		req := &routerrpc.SendPaymentRequest{
			...
			PaymentHash:    ht.Random32Bytes(),
			FinalCltvDelta: chainreg.DefaultLitecoinTimeLockDelta,
			TimeoutSeconds: 60,
			FeeLimitMsat:   noFeeLimitMsat,
		}
		alice.RPC.SendPayment(req)
	...
	var totalFeeCarol ltcutil.Amount
	...
	carolExpectedBalance := ltcutil.Amount(carolStartingBalance) +
		pushAmt - totalFeeCarol

	// In addition, if this is an anchor-enabled channel, further add the
	// anchor size.
	if lntest.CommitTypeHasAnchors(channelType) {
		carolExpectedBalance += ltcutil.Amount(anchorSize)
	}

	require.Equal(ht, carolExpectedBalance,
		ltcutil.Amount(carolBalResp.ConfirmedBalance),
