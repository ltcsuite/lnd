...
func testUpdateChannelPolicy(ht *lntest.HarnessTest) {
	const (
		defaultFeeBase       = 1000
		defaultFeeRate       = 1
		defaultTimeLockDelta = chainreg.DefaultLitecoinTimeLockDelta
		defaultMinHtlc       = 1000
	)
	defaultMaxHtlc := lntest.CalculateMaxHtlc(funding.MaxLtcFundingAmount)

	chanAmt := ltcutil.Amount(100000000)
	pushAmt := chanAmt / 2
...
func testSendUpdateDisableChannel(ht *lntest.HarnessTest) {
	...
	// Give Eve some coins.
	ht.FundCoins(ltcutil.SatoshiPerBitcoin, eve)
	...
	expectedPolicy := &lnrpc.RoutingPolicy{
		FeeBaseMsat:      int64(chainreg.DefaultLitecoinBaseFeeMSat),
		FeeRateMilliMsat: int64(chainreg.DefaultLitecoinFeeRate),
...
func testUpdateChannelPolicyForPrivateChannel(ht *lntest.HarnessTest) {
	...
	timeLockDelta := uint32(chainreg.DefaultLitecoinTimeLockDelta)
