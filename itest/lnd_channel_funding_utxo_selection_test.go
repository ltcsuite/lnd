...
func testChannelUtxoSelection(ht *lntest.HarnessTest) {
	...
	reserveAmount := ltcutil.Amount(reserveResp.RequiredReserve)
	...
	var tcs = []*chanFundUtxoSelectionTestCase{
		...
		// miner fees.
		{
			...
			initialCoins:       []ltcutil.Amount{2_000},
			selectedCoins:      []ltcutil.Amount{2_000},
			chanOpenShouldFail: true,
			feeRate:            15,
			expectedErrStr: "output amount(0.00000174 LTC) after " +
				"subtracting fees(0.00001826 LTC) below dust " +
				"limit(0.00000330 LTC)",
		},
		// Selected coins don't cover the minimum channel size.
		{
			name: "fundmax, local amount < min chan " +
				"size",
			initialCoins:       []ltcutil.Amount{18_000},
			selectedCoins:      []ltcutil.Amount{18_000},
			feeRate:            1,
			chanOpenShouldFail: true,
			expectedErrStr: "available funds(0.00017877 LTC) " +
				"below the minimum amount(0.00020000 LTC)",
		},
		// The local amount exceeds the value of the selected coins.
		{
			name: "selected, local amount > " +
				"selected amount",
			initialCoins:       []ltcutil.Amount{100_000, 50_000},
			selectedCoins:      []ltcutil.Amount{100_000},
			localAmt:           ltcutil.Amount(210_337),
			chanOpenShouldFail: true,
			expectedErrStr: "not enough witness outputs to " +
				"create funding transaction, need 0.00210337 " +
				"LTC only have 0.00100000 LTC available",
		},
		...
		// unselected coin as remaining wallet balance.
		{
			...
			localAmt:        ltcutil.Amount(250_000),
			expectedBalance: ltcutil.Amount(250_000),
			remainingWalletBalance: ltcutil.Amount(350_000) -
				ltcutil.Amount(250_000) -
				fundingFee(2, true),
		},
		...
		// the unselected coin as remaining wallet balance.
		{
			...
			expectedBalance: ltcutil.Amount(200_000) +
				ltcutil.Amount(50_000) -
				fundingFee(2, false),
			remainingWalletBalance: ltcutil.Amount(100_000),
		},
		...
		// local amount to fund an anchor channel.
		{
			...
			selectedCoins:  []ltcutil.Amount{200_000, 100_000},
			commitmentType: lnrpc.CommitmentType_ANCHORS,
			localAmt: ltcutil.Amount(300_000) -
				reserveAmount -
				fundingFee(2, true),
			expectedBalance: ltcutil.Amount(300_000) -
				reserveAmount -
				fundingFee(2, true),
		...
		// enough funds in the wallet to cover for the anchor reserve.
		{
			...
			selectedCoins:  []ltcutil.Amount{200_000},
			commitmentType: lnrpc.CommitmentType_ANCHORS,
			expectedBalance: ltcutil.Amount(200_000) -
				fundingFee(1, false),
			remainingWalletBalance: reserveAmount,
