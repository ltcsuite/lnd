...
func TestLinkForwardMaxHTLCPolicyMismatch(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		ltcutil.SatoshiPerBitcoin*5, ltcutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
...
func newSingleLinkTestHarness(chanAmt, chanReserve ltcutil.Amount) (
...
func TestChannelLinkUpdateCommitFee(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		aliceInitialBalance, ltcutil.SatoshiPerBitcoin*5,
	)
	if err != nil {
		t.Fatalf("unable to create channel: %v", err)
	}
