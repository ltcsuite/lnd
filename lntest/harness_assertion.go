...
func (h *HarnessTest) AssertOutputScriptClass(tx *ltcutil.Tx,
...
func (h *HarnessTest) WaitForBalanceConfirmed(hn *node.HarnessNode,
	expected ltcutil.Amount) {

	var lastBalance ltcutil.Amount
	err := wait.NoError(func() error {
		resp := hn.RPC.WalletBalance()

		lastBalance = ltcutil.Amount(resp.ConfirmedBalance)
...
func (h *HarnessTest) WaitForBalanceUnconfirmed(hn *node.HarnessNode,
	...
	err := wait.NoError(func() error {
		...
		lastBalance = ltcutil.Amount(resp.UnconfirmedBalance)
		if lastBalance == expected {
			return nil
		}
...
func (h *HarnessTest) DecodeAddress(addr string) ltcutil.Address {
	resp, err := ltcutil.DecodeAddress(addr, harnessNetParams)
...
// there's no error.
func (h *HarnessTest) PayToAddrScript(addr ltcutil.Address) []byte {
...
func (h *HarnessTest) CreateBurnAddr(addrType lnrpc.AddressType) ([]byte,
	...
	case lnrpc.AddressType_WITNESS_PUBKEY_HASH:
		addr, err = ltcutil.NewAddressWitnessPubKeyHash(
			ltcutil.Hash160(randomKeyBytes), harnessNetParams,
		)
	...
	case lnrpc.AddressType_NESTED_PUBKEY_HASH:
		var witnessAddr ltcutil.Address
		witnessAddr, err = ltcutil.NewAddressWitnessPubKeyHash(
			ltcutil.Hash160(randomKeyBytes), harnessNetParams,
		)
		require.NoError(h, err)

		addr, err = ltcutil.NewAddressScriptHash(
