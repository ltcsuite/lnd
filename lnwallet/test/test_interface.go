...
import (
	...
	btcdnotify "github.com/ltcsuite/lnd/chainntnfs/btcdnotify"
...
var (
	...
	// litecoind server. It should be used atomically.
...
func loadTestCredits(miner *rpctest.Harness, w *lnwallet.LightningWallet,
	...
	satoshiPerOutput, err := ltcutil.NewAmount(btcPerOutput)
	if err != nil {
	...
	expectedBalance += ltcutil.Amount(int64(satoshiPerOutput) * int64(numOutputs))
	addrs := make([]ltcutil.Address, 0, numOutputs)
...
// createTestWallet creates a test LightningWallet will a total of 20LTC
...
func createTestWallet(tempTestDir string, miningNode *rpctest.Harness,
	...
	cfg := lnwallet.Config{
		...
		DefaultConstraints: channeldb.ChannelConstraints{
			DustLimit:        5000,
			MaxPendingAmount: lnwire.NewMSatFromSatoshis(ltcutil.SatoshiPerBitcoin) * 100,
			ChanReserve:      100,
			MinHTLC:          400,
...
func testDualFundingReservationWorkflow(miner *rpctest.Harness,
	...
	fundingAmount, err := ltcutil.NewAmount(5)
...
func testFundingTransactionLockedOutputs(miner *rpctest.Harness,
	...
	fundingAmount, err := ltcutil.NewAmount(8)
	require.NoError(t, err, "unable to create amt")
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	require.NoError(t, err, "unable to query fee estimator")
...
func testFundingCancellationNotEnoughFunds(miner *rpctest.Harness,
	...
	fundingAmount, err := ltcutil.NewAmount(44)
	require.NoError(t, err, "unable to create amt")
	req := &lnwallet.InitFundingReserveMsg{
		ChainHash:        chainHash,
		NodeID:           bobPub,
...
func testCancelNonExistentReservation(miner *rpctest.Harness,
	...
	res, err := lnwallet.NewChannelReservation(
		11812, 11812, alice, 22, &testHdSeed, 0, req,
	)
...
func testReservationInitiatorBalanceBelowDustCancel(miner *rpctest.Harness,
	...
	const numBTC = 4
	fundingAmount, err := ltcutil.NewAmount(numBTC)
	require.NoError(t, err, "unable to create amt")

	feePerKw := chainfee.SatPerKWeight(
		numBTC * numBTC * ltcutil.SatoshiPerBitcoin,
	)
	req := &lnwallet.InitFundingReserveMsg{
		ChainHash:        chainHash,
		NodeID:           bobPub,
...
func testSingleFunderReservationWorkflow(miner *rpctest.Harness,
	...
	// towards Bob's side.
	fundingAmt, err := ltcutil.NewAmount(4)
	require.NoError(t, err, "unable to create amt")
	pushAmt := lnwire.NewMSatFromSatoshis(ltcutil.SatoshiPerBitcoin)
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	require.NoError(t, err, "unable to query fee estimator")
	aliceReq := &lnwallet.InitFundingReserveMsg{
		ChainHash:        chainHash,
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	const numTxns = 5
	const outputAmt = ltcutil.SatoshiPerBitcoin
...
func scriptFromKey(pubkey *btcec.PublicKey) ([]byte, error) {
	...
	keyAddr, err := ltcutil.NewAddressWitnessPubKeyHash(
...
func txFromOutput(tx *wire.MsgTx, signer input.Signer, fromPubKey,
	payToPubKey *btcec.PublicKey, txFee ltcutil.Amount,
...
func newTx(t *testing.T, r *rpctest.Harness, pubKey *btcec.PublicKey,
	...
	txFee := ltcutil.Amount(0.001 * ltcutil.SatoshiPerBitcoin)
...
func testPublishTransaction(r *rpctest.Harness,
	...
	// transactions.
	var (
		txFee         = ltcutil.Amount(0.005 * ltcutil.SatoshiPerBitcoin)
	...
	for _, rbf := range []bool{false, true} {
		...
		// TODO(litecoin): figure out why this breaks on ltcd/neutrino
		if alice.BackEnd() != "litecoind" {
			if rbf {
	...
	// backing ltcd node will consider the tx being an orphan, and will
	// accept it. Should look into if this is the behavior also for
	// litecoind, and update test accordingly.
...
func testSignOutputUsingTweaks(r *rpctest.Harness,
	...
	for i := 0; i < 2; i++ {
		...
		pubkeyHash := ltcutil.Hash160(tweakedKey.SerializeCompressed())
		keyAddr, err := ltcutil.NewAddressWitnessPubKeyHash(pubkeyHash,
		...
		vm, err := txscript.NewEngine(
			...
			nil, nil, int64(ltcutil.SatoshiPerBitcoin),
			txscript.NewCannedPrevOutputFetcher(
				keyScript, int64(ltcutil.SatoshiPerBitcoin),
			),
		)
		if err != nil {
...
func testChangeOutputSpendConfirmation(r *rpctest.Harness,
	...
	output = &wire.TxOut{
		Value:    ltcutil.SatoshiPerBitcent,
		PkScript: bobPkScript,
	}
	tx = sendCoins(t, r, alice, bob, output, txFeeRate, true, 1)
	txHash = tx.TxHash()
...
func testSpendUnconfirmed(miner *rpctest.Harness,
	...
	output = &wire.TxOut{
		Value:    ltcutil.SatoshiPerBitcoin * 0.5,
		PkScript: alicePkScript,
	}
	...
	output = &wire.TxOut{
		Value:    int64(bobBalance) - (ltcutil.SatoshiPerBitcoin * 0.4),
		PkScript: bobPkScript,
	}
...
func testSingleFunderExternalFundingTx(miner *rpctest.Harness,
	...
	// to Bob's side.
	chanAmt := 4 * ltcutil.SatoshiPerBitcoin
...
func runTests(t *testing.T, walletDriver *lnwallet.WalletDriver,
	...
	case "ltcwallet":
		...
		case "neutrino":
			...
			// instance, and initialize a ltcwallet driver for it.
		...
		case "litecoind":
			...
			err = bitcoind.Start()
			if err != nil {
				t.Fatalf("couldn't start litecoind: %v", err)
			}

			// Sanity check to ensure that the process did in fact
			// start.
			if bitcoind.Process == nil {
				t.Fatalf("litecoind cmd Process is not set " +
					"after Start")
			}
			...
			// Wait for the litecoind instance to start up.
			...
			// Create a btcwallet litecoind client for both Alice and
			// Bob.
		...
		if err != nil {
			t.Fatalf("unable to create ltcwallet: %v", err)
		}
