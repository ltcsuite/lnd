...
func testAddSettleWorkflow(t *testing.T, tweakless bool,
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(ltcutil.SatoshiPerBitcoin)
	...
	mSatTransferred := lnwire.NewMSatFromSatoshis(ltcutil.SatoshiPerBitcoin)
...
func TestCheckCommitTxSize(t *testing.T) {
	...
	checkSize := func(channel *LightningChannel, count int) {
		...
		actualCost := blockchain.GetTransactionWeight(ltcutil.NewTx(commitTx))
...
func TestHTLCDustLimit(t *testing.T) {
	...
	// Bob's dust limit.
	htlcSat := (ltcutil.Amount(500) + HtlcTimeoutFee(
...
func TestHTLCSigNumber(t *testing.T) {
	...
	createChanWithHTLC := func(htlcValues ...ltcutil.Amount) (
	...
	belowDust := ltcutil.Amount(500) + HtlcTimeoutFee(
		channeldb.SingleFunderTweaklessBit, feePerKw,
	)
	aboveDust := ltcutil.Amount(1400) + HtlcSuccessFee(
		channeldb.SingleFunderTweaklessBit, feePerKw,
	)
...
func TestCancelHTLC(t *testing.T) {
	...
	// of the new HTLC.
	aliceExpectedBalance := ltcutil.Amount(ltcutil.SatoshiPerBitcoin*4) -
	...
	expectedBalance := ltcutil.Amount(ltcutil.SatoshiPerBitcoin * 5)
...
func TestCooperativeCloseDustAdherence(t *testing.T) {
	...
	setDustLimit := func(dustVal ltcutil.Amount) {
	...
	// satoshis.
	dustLimit := ltcutil.Amount(10000)
	...
	aliceFee := ltcutil.Amount(aliceChannel.CalcFee(aliceFeeRate)) + 1000
	aliceSig, _, _, err := aliceChannel.CreateCloseProposal(
		aliceFee, aliceDeliveryScript, bobDeliveryScript,
	)
	require.NoError(t, err, "unable to close channel")

	bobFee := ltcutil.Amount(bobChannel.CalcFee(bobFeeRate)) + 1000
	bobSig, _, _, err := bobChannel.CreateCloseProposal(
		bobFee, bobDeliveryScript, aliceDeliveryScript,
	)
	require.NoError(t, err, "unable to close channel")
	...
	aliceBal := lnwire.NewMSatFromSatoshis(ltcutil.SatoshiPerBitcoin)
...
func TestAddHTLCNegativeBalance(t *testing.T) {
	...
	htlcAmt = lnwire.NewMSatFromSatoshis(2 * ltcutil.SatoshiPerBitcoin)
...
func TestFeeUpdateOldDiskFormat(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(0.1 * ltcutil.SatoshiPerBitcoin)
	const numHTLCs = 30
...
func TestChanAvailableBalanceNearHtlcFee(t *testing.T) {
	...
	aliceBalance := lnwire.NewMSatFromSatoshis(5 * ltcutil.SatoshiPerBitcoin)
	bobBalance := lnwire.NewMSatFromSatoshis(5 * ltcutil.SatoshiPerBitcoin)
...
func TestDesyncHTLCs(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromSatoshis(4.1 * ltcutil.SatoshiPerBitcoin)
	htlc, _ := createHTLC(0, htlcAmt)
	...
	htlcAmt = lnwire.NewMSatFromSatoshis(1 * ltcutil.SatoshiPerBitcoin)
...
func TestMaxPendingAmount(t *testing.T) {
	...
	maxPending := lnwire.NewMSatFromSatoshis(ltcutil.SatoshiPerBitcoin * 3)
	...
	const numHTLCs = 2
	htlcAmt := lnwire.NewMSatFromSatoshis(1.5 * ltcutil.SatoshiPerBitcoin)
	for i := 0; i < numHTLCs; i++ {
		htlc, _ := createHTLC(i, htlcAmt)
		if _, err := aliceChannel.AddHTLC(htlc, nil); err != nil {
			t.Fatalf("unable to add htlc: %v", err)
...
func assertChannelBalances(t *testing.T, alice, bob *LightningChannel,
	aliceBalance, bobBalance ltcutil.Amount) {
...
func TestChanReserve(t *testing.T) {
	...
	setupChannels := func() (*LightningChannel, *LightningChannel) {
		...
		aliceMinReserve := ltcutil.Amount(0.5 *
			ltcutil.SatoshiPerBitcoin)
		...
		bobMinReserve := ltcutil.Amount(6 * ltcutil.SatoshiPerBitcoin)
	...
	//	Alice:	4.5
	//	Bob:	5.0
	htlcAmt := lnwire.NewMSatFromSatoshis(0.5 * ltcutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		...
		ltcutil.SatoshiPerBitcoin*4.5-commitFee, ltcutil.SatoshiPerBitcoin*5,
	)
	...
	//	Alice:	1.5
	//	Bob:	9.5
	htlcAmt = lnwire.NewMSatFromSatoshis(3.5 * ltcutil.SatoshiPerBitcoin)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		ltcutil.SatoshiPerBitcoin*3-commitFee, ltcutil.SatoshiPerBitcoin*5,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		ltcutil.SatoshiPerBitcoin*3-commitFee, ltcutil.SatoshiPerBitcoin*7,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		ltcutil.SatoshiPerBitcoin*3-commitFee, ltcutil.SatoshiPerBitcoin*6,
	)
}
...
func TestChanReserveRemoteInitiator(t *testing.T) {
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*ltcutil.SatoshiPerBitcoin - commitFee
...
func TestChanReserveLocalInitiatorDustHtlc(t *testing.T) {
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*ltcutil.SatoshiPerBitcoin - commitFee - htlcSat
...
func TestMinHTLC(t *testing.T) {
	...
	minValue := lnwire.NewMSatFromSatoshis(0.1 * ltcutil.SatoshiPerBitcoin)
...
func TestChannelMaxFeeRate(t *testing.T) {
	...
	assertMaxFeeRate(anchorChannel, 1.0, 435936270)
	assertMaxFeeRate(anchorChannel, 0.001, 435936)
...
func TestIdealCommitFeeRate(t *testing.T) {
	...
	t.Run("anchor-channel", func(t *testing.T) {
		...
		// maximum fee.
		assertIdealFeeRate(
			anchorChannel, 700000000, 0, 1000000, 0.001, 435936,
		)
...
func TestMayAddOutgoingHtlc(t *testing.T) {
	...
	chanBal, err := ltcutil.NewAmount(testChannelCapacity)
	require.NoError(t, err)
