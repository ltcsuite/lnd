...
const (
	...
	minerLogFilename = "output_ltcd_miner.log"
...
// NewMiner creates a new miner using ltcd backend with the default log file
// dir and name.
...
// NewTempMiner creates a new miner using ltcd backend with the specified log
// file dir and name.
...
// newMiner creates a new miner using ltcd's rpctest.
...
func (h *HarnessMiner) saveLogs() {
	...
	for _, file := range files {
		newFilename := strings.Replace(
			file.Name(), "ltcd.log", h.logFilename, 1,
		)
...
// asserts.
func (h *HarnessMiner) GetRawTransaction(txid *chainhash.Hash) *ltcutil.Tx {
...
func (h *HarnessMiner) SendOutputsWithoutChange(outputs []*wire.TxOut,
	feeRate ltcutil.Amount) *chainhash.Hash {
...
func (h *HarnessMiner) CreateTransaction(outputs []*wire.TxOut,
	feeRate ltcutil.Amount) *wire.MsgTx {
...
func (h *HarnessMiner) NewMinerAddress() ltcutil.Address {
...
// transactions only.
func (h *HarnessMiner) MineBlockWithTxes(txes []*ltcutil.Tx) *wire.MsgBlock {
