...
func testSphinxReplayPersistence(ht *lntest.HarnessTest) {
	...
	ht.FundCoins(ltcutil.SatoshiPerBitcoin, fred)
...
func testMaxPendingChannels(ht *lntest.HarnessTest) {
	...
	amount := funding.MaxLtcFundingAmount
...
func testGarbageCollectLinkNodes(ht *lntest.HarnessTest) {
	...
	// closed.
	ht.MineBlocks(
		chainreg.DefaultLitecoinTimeLockDelta - defaultCSV,
	)
...
func testSendAllCoins(ht *lntest.HarnessTest) {
	...
	ht.FundCoins(ltcutil.SatoshiPerBitcoin, ainz)
	ht.FundCoinsNP2WKH(ltcutil.SatoshiPerBitcoin, ainz)
	...
	err = ainz.RPC.SendCoinsAssertErr(&lnrpc.SendCoinsRequest{
		Addr:       "tltc1qfc8fusa98jx8uvnhzavxccqlzvg749tvtx95mp",
		SendAll:    true,
		Label:      sendCoinsLabel,
		TargetConf: 6,
	})
	...
	err = ainz.RPC.SendCoinsAssertErr(&lnrpc.SendCoinsRequest{
		Addr:       "LfcXnY7uNN7PdYBc1iKQwDkzFfsFw1oPCC",
		SendAll:    true,
		Label:      sendCoinsLabel,
		TargetConf: 6,
	})
