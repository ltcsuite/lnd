...
func randPubKeyHashScript() ([]byte, *btcec.PrivateKey, error) {
	...
	addrScript, err := ltcutil.NewAddressPubKeyHash(pubKeyHash, NetParams)
	if err != nil {
		return nil, nil, err
	}
...
// NewMiner spawns testing harness backed by a ltcd node that can serve as a
// miner.
...
func NewBitcoindBackend(t *testing.T, minerAddr string, txindex,
	...
	bitcoind := exec.Command("litecoind", args...)
...
func NewNeutrinoBackend(t *testing.T, minerAddr string) *neutrino.ChainService {
	...
	// Create an instance of neutrino connected to the running ltcd
	// instance.
	...
	// generated by the ltcd instance.
	spvNode.Start()
