...
func (i testInputType) output(t *testing.T,
	...
	switch i {
	case plainP2WKH:
		h := ltcutil.Hash160(privKey.PubKey().SerializeCompressed())
		addr, err = ltcutil.NewAddressWitnessPubKeyHash(h, netParams)
	...
	case singleKeyP2WSH:
		...
		addr, err = ltcutil.NewAddressWitnessScriptHash(h[:], netParams)
		require.NoError(t, err)
