...
func testOnchainFundRecovery(ht *lntest.HarnessTest) {
	...
	skipAndSend := func(nskip int) func(*node.HarnessNode) {
		return func(node *node.HarnessNode) {
			...
			ht.FundCoins(ltcutil.SatoshiPerBitcoin, node)

			// And another to the next NP2WKH address.
			ht.FundCoinsNP2WKH(ltcutil.SatoshiPerBitcoin, node)

			// Add another whole coin to the P2TR address.
			ht.FundCoinsP2TR(ltcutil.SatoshiPerBitcoin, node)
		}
	}
	...
	// derivation path.
	restoreCheckBalance(3*ltcutil.SatoshiPerBitcoin, 3, 1, skipAndSend(9))

	// Check that using a recovery window of 9 does not find the two most
	// recent txns.
	restoreCheckBalance(3*ltcutil.SatoshiPerBitcoin, 3, 9, nil)
	...
	restoreCheckBalance(6*ltcutil.SatoshiPerBitcoin, 6, 10, skipAndSend(19))

	// Check that recovering with a recovery window of 19 fails to find the
	// most recent transactions.
	restoreCheckBalance(6*ltcutil.SatoshiPerBitcoin, 6, 19, nil)
	...
	const minerAmt = 8 * ltcutil.SatoshiPerBitcoin
	const finalBalance = 9 * ltcutil.SatoshiPerBitcoin
...
func testRescanAddressDetection(ht *lntest.HarnessTest) {
	...
	pubKeyHash := ltcutil.Hash160(keyDesc.RawKeyBytes)
	ghostUtxoAddr, err := ltcutil.NewAddressWitnessPubKeyHash(
